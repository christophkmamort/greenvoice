...
class Brand(models.Model):
    ...
    def save(self, *args, **kwargs):
        logo = Image.open(self.branding.logo)
        width = 320
        if logo.size[0] > width:
            ...
            inmemory_uploaded_file = InMemoryUploadedFile(thumb_io, None, self.branding.logo.name.split(
                '.')[0] + '.jpg', 'logo/jpeg', thumb_io.tell(), None)
            self.branding.logo = inmemory_uploaded_file
...
class BrandSettings(models.Model):
    ...
    free_shipping_from = models.FloatField(blank=True, verbose_name=_('free shipping from'))
    return_policy_period = models.IntegerField(blank=True, verbose_name=_('return policy period'))
...
class BrandSettingsSales(models.Model):
    ...
    amount = models.FloatField(blank=True, verbose_name=_('amount (in %)'))
    from_datetime = models.DateTimeField(blank=True, verbose_name=_('status from'))
    until_datetime = models.DateTimeField(blank=True, verbose_name=_('status until'))
